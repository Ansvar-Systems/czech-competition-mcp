# ─────────────────────────────────────────────────────────────────────────────
# Czech Competition (UOHS) MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t czech-competition-mcp .
# Run:    docker run --rm -p 3000:3000 czech-competition-mcp
#
# The image bakes /app/data/uohs.db at build time. Override CZ_COMP_DB_PATH
...
ENV CZ_COMP_DB_PATH=/app/data/uohs.db
...
# Bake the pre-built database into the image so /app/data/uohs.db resolves
...
# We then COPY it into the image at /app/data/uohs.db (CZ_COMP_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/uohs.db
